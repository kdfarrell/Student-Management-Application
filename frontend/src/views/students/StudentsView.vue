...
<template>
			...
			<TableRow v-for="student in studentStore.students" :key="student.id">
				...
				<TableCell>
					<div class="flex items-center justify-center gap-2">
						...
						<button class="p-1 hover:text-blue-500 transition-colors cursor-pointer"
							@click="handleViewDetail(student)">
							<Eye class="w-4 h-4" />
						</button>
						<button class="p-1 hover:text-yellow-500 transition-colors cursor-pointer"
							@click="selectedStudent = student; openModal = true">
							<SquarePen class="w-4 h-4" />
						</button>
						<button class="p-1 hover:text-red-500 transition-colors cursor-pointer"
	...
	<div class="flex items-center justify-end mt-4">
		<Pagination v-slot="{ page }" :total="studentStore.count" :items-per-page="studentStore.pageSize || 10"
			:page="studentStore.currentPage" @update:page="studentStore.goToPage($event)">
	...
	<StudentModal :student="selectedStudent" :open="openModal" @update:open="openModal = $event" />

	<StudentDeleteConfirm :student="selectedStudent" :open="openDelete" @update:open="openDelete = $event" />

	<StudentDetail :student="selectedStudent" :open="openDetail" @update:open="openDetail = $event" />
</template>