...
<template>
	<div class="space-y-6">
		<div class="flex justify-between items-center">
			<p class="text-sm text-muted-foreground">Select a class session below to log or modify student attendance
				metrics.</p>
		</div>

		<!-- Active Class Sessions Grid -->
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			<Card v-for="session in scheduleStore.schedules" :key="session.id"
				...
				<CardHeader class="pb-3 flex flex-row items-start justify-between space-y-0">
					...
					<Badge :variant="session.attendance_recorded ? 'default' : 'secondary'"
		...
		<AttendanceSheet v-model:open="isSheetOpen" :session="selectedSession"
