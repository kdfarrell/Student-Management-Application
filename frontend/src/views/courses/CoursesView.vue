...
import { BookOpen, Users, SquarePen, Trash2 } from 'lucide-vue-next'
...
<template>
        ...
        <Card v-for="course in courses" :key="course.id" class="hover:shadow-lg transition-shadow duration-200">
            <CardHeader class="pb-2">
                <div class="flex items-center justify-between">
                    ...
                    <span :class="course.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" class="text-xs px-2 py-1 rounded-full font-medium">
            ...
            <CardFooter class="pt-2 flex gap-2">
                <Button class="cursor-pointer flex-1" @click="$router.push(`/courses/${course.id}`)">View Details</Button>
                <Button class="cursor-pointer" variant="outline" size="icon" @click="selectedCourse = course; openModal = true">
    ...
    <CourseModal 
        ...
        @saved="courseStore.fetchCourses()" 
    />
