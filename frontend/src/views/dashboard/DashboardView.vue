...
const attendanceChartData = computed(() => {
	if (!courseAttendance.value.length) return null
	return {
		labels: courseAttendance.value.map(c => c.name),
		datasets: [{
			data: courseAttendance.value.map(c => c.attendance_percentage ?? 0),
...
const doughnutChartOptions = {
	...
	cutout: '72%',
	plugins: {
		legend: { position: 'bottom', labels: { color: '#a1a1aa', padding: 12, boxWidth: 10, font: { size: 11 } } },
		tooltip: TOOLTIP_STYLE,
	},
}
...
function courseAttendanceColor(pct) {
	if (pct >= 75) return 'bg-foreground'
			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
				...
				<Card>
					<CardHeader class="pb-3">
						<CardTitle class="text-sm font-semibold">Attendance by Course</CardTitle>
						<CardDescription class="text-xs mt-0.5">Average % per course</CardDescription>
					</CardHeader>
					<CardContent class="flex flex-col items-center justify-center gap-2">
						<div class="w-44 h-44">
							<Doughnut v-if="attendanceChartData" :data="attendanceChartData"
