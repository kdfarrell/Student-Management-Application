	...
	<div class="p-6">
		...
		<div class="flex gap-6">
			...
			<div class="w-64 shrink-0 space-y-4">
				...
				<div v-if="selectedCourseId">
					...
					<Select v-model="selectedSubjectId">
						...
						<SelectContent>
							<SelectItem v-for="subject in filteredSubjects" :key="subject.id"
			...
			<div class="flex-1">
				...
				<div v-else>
					<AssessmentList :subject-id="Number(selectedSubjectId)" @select="handleAssessmentSelect" />

					<GradeEntry v-if="selectedAssessmentId" :assessment-id="selectedAssessmentId"
						:course-id="Number(selectedCourseId)" class="mt-6" />
				</div>
			</div>
		</div>
	</div>
